...
class urlSokucu {
  constructor(url,sunucu,token) {
   ...
   console.log("\x1b[31m%s\x1b[0m",'[URL SPAMMER] Girilen token yanlış veya boşluklu girilmiş lütfen kontrol edin.');
   })
   client.on("ready", () => {
     const guild = client.guilds.cache.get(sunucu)
     console.log("\x1b[33m%s\x1b[0m",`[URL SPAMMER] [${guild.name}] sunucusuna ${url} adlı URL Spamlanmaya başlayacak.`)
     setInterval(async () => {
       if(guild.vanityURLCode == url) {
        ...
        if(!guild.channels.cache.find(k => k.name === "url-log")) { await guild.channels.create('url-log', { type: 'text', reason: "Url Başarıyla Alındı!"}).then(async kanal => { await kanal.send(mesaj)}) } else {
         ...
         console.log("\x1b[32m%s\x1b[0m",`[URL SPAMMER] [${guild.name}] isimli sunucuya "${guild.vanityURLCode}" başarıyla alındı.`) 
         process.exit();
        } else {
